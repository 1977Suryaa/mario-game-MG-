...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("press the start button and use your nost to control ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function start(){
  game_status="loading"
  document.getElementById("checking").innerHTML="game is loading"
} 


// change game status if any key is pressed
function changeGameStatud(character){
  if( noseX !=""&& gameConfig.status==="start"&&game_status=="loading") {
    world_start.play();
